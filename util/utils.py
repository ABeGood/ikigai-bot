...
def format_prepay(sum: float):
    return f'''Мы принимаем онлайн оплату через систему *Revolut*.
Клик по кнопке *"Оплатить 🪙"* неренаправит Вас на *страницу оплаты*.
*Сумма*: *{sum}* CZK
Нам будет удобно, если в примечании Вы напишете *свое имя*.
После оплаты нажмите на "Готово ✅".
Спасибо! ✨
'''
